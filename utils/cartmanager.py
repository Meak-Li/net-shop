    ...
    def migrateSession2DB(self):
        if 'user' in self.session:
            ...
            for cartitem in self.queryAll():
                if CartItem.objects.filter(goodsid=cartitem.goodsid,colorid=cartitem.colorid,sizeid=cartitem.sizeid,user=user).count()==0:
                    cartitem.user = user
                    cartitem.save()
                else:
                    item = CartItem.objects.get(goodsid=cartitem.goodsid,colorid=cartitem.colorid,sizeid=cartitem.sizeid,user=user)
